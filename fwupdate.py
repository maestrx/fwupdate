...
#===============================================================================
#
# USAGE:        fwupdate.py -f <fw_file> [ -v | -h | -s | -p serial | --force ] 
#
# DESCRIPTION:
#               This script uploads firmware to UPiS. Only mandatory input is new UPiS firmware.   
#
# RETURN CODES:
#               0 - Sucessfull update   
#               1 - Failed to parse command line arguments   
#               2 - Failed to establish communication with the UPiS   
#               3 - Incompatible UPiS powering mode (DISABLED FOR NOW)   
...
#               python-serial
#               python-smbus
#               Propoer HW setup and setup of Pi to enable Serial/I2C communication based on the UPiS manual
# BUGS:         ---
# NOTES:        ---
# AUTHOR:       Vit SAFAR <upis@safar.info>
# VERSION:      1.3
# CREATED:      2.6.2014
# REVISION: 
...
#               - Show UPiS @status after firmware update 
#               - Detect progress of the factory reset, not just wait :)
#               - Set UPIS RTC clock after factory reset to the system time
#
#===============================================================================
import sys
import time
...
class FWUpdate(object):
  """ 
    ...
    The class performs following tasks
      ...
      4) Verify the connectivity to UPiS bootloader is working
      5) Perform FW update
      6) Perform UPiS factory reset
  ...
  # detected i2c bus  
  i2c_bus=False
  # default I2C port of UPiS control interface
  i2c_port=0x69
  ...
  def power_detect(self):
      ...
      try:
        ...
        if pwr>0:
          ...
          # if forced to skip this check, lets do it :)
            ...
            print 'WARNING: (I2C-0) System is not powered via Pi USB port. There is a UPiS reset after a FW update, that would perform hard reset of Pi! (use --force to disable this check)'
            self.power_warned=True
          else:
            print 'ERROR: (I2C-0) System has to be powered via the Pi USB port. There is a UPiS reset after a FW update, that would perform hard reset of Pi! (use --force to disable this check)'
            sys.exit(3)
        ...
        try:  
          ...
          if pwr>0:
            ...
            # if forced to skip this check, lets do it :)
              ...
              print 'WARNING: (I2C-1) System is not powered via Pi USB port. There is a UPiS reset after a FW update, that would perform hard reset of Pi! (use --force to disable this check)'
              self.power_warned=True
            else:
              print 'ERROR: (I2C-1) System has to be powered via the Pi USB port. There is a UPiS reset after a FW update, that would perform hard reset of Pi! (use --force to disable this check)'
              sys.exit(3)
        except SystemExit as e:
          sys.exit(e)      
        except:
    ...
    if not self.power and not self.power_warned:
      ...
      # Set up the connection to the upis
      upis = self.serial.Serial(port=self.port,baudrate=38400,timeout=0,rtscts=0,xonxoff=0)
      # empty the input buffer
      for line in upis:
        pass
      # get status of power via serial from upis
      upis.write('@PM\r')
      # wait for the answer
      time.sleep(0.5)
        ...
        if line[:16] == 'Powering Source:':    
          ...
          # if forced to skip this check, lets do it :)
            ...
            if not self.power_warned:
              print 'WARNING: (Serial) System is not powered via Pi USB port. There is a UPiS reset after a FW update, that would perform hard reset of Pi! (use --force to disable this check)'
              self.power_warned=True
          else:
            print 'ERROR: (Serial) System has to be powered via the Pi USB port. There is a UPiS reset after a FW update, that would perform hard reset of Pi! (use --force to disable this check)'
            sys.exit(3)
      # close the connection to UPIS via serial
      upis.close()
    ...
    if not self.power:
      if self.force:
        if not self.power_warned:
          print 'WARNING: System powering mode not detected. There is a UPiS reset after a FW update, that would perform hard reset of Pi! Use --force to disable this check.'
      else:
        print 'ERROR: System powering mode not detected. System has to be powered via the Pi USB port since here is a UPiS reset after a FW update, that would perform hard reset of Pi! Make a proper HW/Pi setup of Serial interface or PiCo interface(I2C) to enable auto-detection. This can happen also in case that UPiS is already in the bootload mode having UPiS RED led lid. Use --force to disable this check.'
        sys.exit(3)
       

  """
  3) Check that there is a UPiS bootloader connected to the other side of the serial interface :)
  ...
  def serial_check(self):
    ...
    try:
      # Set up the connection to the upis
      upis = self.serial.Serial(port=self.port,baudrate=38400,timeout=0,rtscts=0,xonxoff=True)
      # empty the input buffer
      for line in upis:
        pass
      # send dummy command
      upis.write(":020000040000FA\r")
    except:
      print "KO\nERROR: Unable to establish communication with UPiS bootloader via port:",self.port,'Please verify that the serial port is availble.'
      sys.exit(2)
    try:
      ...
      while rcnt>0:
        ...
        for resp in upis:
          ...
          if ord(resp[0])==6:
            ...
            print "KO\nERROR: Invalid response from UPiS:",ord(resp[0])," Please retry the FW upload process."
            sys.exit(2)
          break
        rcnt-=1
    except:
      print "KO\nERROR: Something wrong happened during verification of communication channel with UPiS bootloader via port:",self.port,'Please verify that the serial port is availble and not used by some other application.'
      sys.exit(2)
      
    # in case communication not verified
    if not status:
      if self.force:
        print "KO\nWARNING: Unable to verify communication with bootloader in UPiS. Is the UPiS in the bootloader mode? (Red LED lid on UPiS)"      
      else:
        print "KO\nERROR: Failed to establish communication with bootloader in UPiS. Is the UPiS in the bootloader mode? (Red LED lid on UPiS)"
        sys.exit(2)
    # close the channel to UPiS
    upis.close()
    ...
    while rcnt>0:
      ...
      for resp in upis:
        ...
        if ord(resp[0])==6:
          ...
          print "KO\nERROR: Invalid status word from UPiS (",ord(resp),') when processing initial line! Please retry the FW upload process.'
          sys.exit(5)
        break
      rcnt-=1
    if not status:
      print "KO\nERROR: No status word from UPiS revcieved when processing initial line! Please check possible warnings above and retry the FW upload process."
      sys.exit(5)
      ...
      while rcnt>0:
        ...
        for resp in upis:
          ...
          if ord(resp[0])==6:
            ...
            print "KO\nERROR: Invalid status word from UPiS (",ord(resp),') when processing line',lnum2,' Please retry the FW upload process.'
            sys.exit(5)
          break
        rcnt-=1
      if not status:
        print "KO\nERROR: No status word from UPiS revcieved when processing line",lnum2,' Please check possible warnings above and retry the FW upload process.'
        sys.exit(5)
      # in case that the done command is detected, than finish
      if line[7:9]=='01':
        break
  ...
  """
  6) Perform factory reset of the UPiS
        
  """
  def factory_reset(self):
    time.sleep(3)
    print "Invoking factory reset of UPiS..."
    status=False
    # Set up the connection to the upis
    upis = self.serial.Serial(port=self.port,baudrate=38400,timeout=0,rtscts=0,xonxoff=0)
    # empty the input buffer
